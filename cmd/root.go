...
func decodeSecret(secertName string, opt options) error {

	client := client.K8Client()
	secret, err := client.CoreV1().Secrets(opt.namespace).Get(context.Background(), secertName, metav1.GetOptions{})
